...
- [x] Follow-up: second dev engine default `faster_whisper`/`tiny` → `gigaam`/`v1_rnnt` + `ASR_REQUEST_LOGGING=true` in both dev compose files; `deploy/dev/.env.example` defaults + new `ASR_REQUEST_LOGGING2`; `deploy/dev/config.yaml` engine renamed `faster` → `gigaam` (header + entry + comment); docs — `ADR-008` (decision/consequences/alternatives), `docs/api/whisper-asr-webservice.md` (new `ASR_MODEL` section: Whisper + GigaAM model names), `CLAUDE.md`, `README.md`, `EPIC-054` follow-up note; `docker compose … config` resolves `ASR_ENGINE=gigaam`/`ASR_MODEL=v1_rnnt`/`ASR_REQUEST_LOGGING=true` on both files (EPIC-054, 2026-09-02)
- [x] Follow-up: `gigaam` diarizes (pyannote) — dropped `diarize: false` on the `gigaam` engine entry in `deploy/dev/config.yaml` (now inherits `diarize: true` / `speaker_timestamps: true` from the base block); base-block comment notes both engines use pyannote/`HF_TOKEN`; docs — `docs/api/whisper-asr-webservice.md` (new "Diarization support by engine" section + dev-stack note), `ADR-008` consequences, `README.md` diarization note, `EPIC-054` follow-up note; `load_config` → `gigaam` diarize=True (EPIC-054, 2026-09-02)
