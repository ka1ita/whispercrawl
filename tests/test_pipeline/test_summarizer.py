...
class TestSummarizeFile:
    def test_returns_ollama_response_content(self):
        cfg = _cfg()
        with patch("asr_crawler.pipeline.summarizer.httpx.post", return_value=_mock_response("Great summary")) as mock_post:
    ...
    def test_sends_prompt_as_system_message(self):
        ...
        with patch("asr_crawler.pipeline.summarizer.httpx.post", return_value=_mock_response()) as mock_post:
    ...
    def test_non_200_raises_summarization_error(self):
        cfg = _cfg()
        with patch("asr_crawler.pipeline.summarizer.httpx.post", return_value=_mock_response(status=500)):
    ...
    def test_request_error_raises_summarization_error(self):
        import httpx
        cfg = _cfg()
        with patch("asr_crawler.pipeline.summarizer.httpx.post", side_effect=httpx.ReadTimeout("timed out")):
...
class TestConcatTranscriptions:
    ...
    def test_does_not_call_ollama(self):
        cfg = _cfg()
        with patch("asr_crawler.pipeline.summarizer.httpx.post") as mock_post:
