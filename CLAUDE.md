...
written (EPIC-051/052/055). `max_error_count` (EPIC-058) stops the run — and
parks every later run until `asr-crawler --reset-errors` — once too many files
fail in a row.
...
- **Max error count** (`max_error_count`, EPIC-058, default `null` = off): a circuit breaker for a systemic failure (ASR/Ollama down, wrong model name, a directory of corrupt media). A `meta.error_count` counter in the index counts **consecutive file failures** across runs (file-count, not file×engine); a fully-successful file resets it to `0`. When it reaches `max_error_count` the current `per_file` run stops after the tripping file (already-finished results kept; the incomplete per-directory pass is skipped) and **every subsequent run** (scheduled, `--once`, `--refresh`) short-circuits at a pre-flight check with an ERROR line and exit 0 until an operator runs `asr-crawler --reset-errors` (zeroes the counter; leaves `errors` rows and the pipeline otherwise untouched). `--dry-run` never counts; `--cleanup` (via `state.clear()`) also zeroes the counter. `per_step` mode finishes the current batch, then the next run is blocked.
