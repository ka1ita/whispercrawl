...
```text
audio/video file
  → Transcriber      → transcript text        (→ index asr_text)
  → PostProcessor    → fixed text             (→ index fixed_text, in memory)
...
- `postprocessing.replace_transcription`, `file_summarization.output_suffix`,
  ...
  `_fix` / `_sum` / `_all` / `_concat` files from a pre-047 catalog.
