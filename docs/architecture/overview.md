...
asr-crawler runs as a scheduled Python process on the local filesystem. It has no inbound network interface — it communicates outbound only to two external services.
...
Each pipeline step catches its own exceptions and records the failure in the processing index: `files.status = 'error'` plus an `errors` row per failing step and engine (`scope` `file` or `dir`, the full message). The pipeline continues to the next file — a single failure does not halt the run, and one engine failing does not block the others. The row is cleared when that file / directory next completes successfully, and dropped when the source file changes (mtime/size). `asr-crawler --errors` prints the outstanding rows grouped by path and exits non-zero when any exist; `asr-crawler --cleanup` empties the table with the rest of the index. Nothing is ever written beside the audio on failure — no `_err.txt` sidecar in any configuration (EPIC-051/052).

### Systemic-failure brake (`max_error_count`, EPIC-058)

A single-failure-tolerant run still needs a stop when *everything* fails (ASR/Ollama down, wrong model, a directory of corrupt media). A `meta.error_count` row in the index counts **consecutive file failures across runs** (one per file, not per engine/step); a fully-successful file resets it to `0`. When it reaches `max_error_count` (config, `null` = disabled) the `per_file` run stops after the tripping file — results already written stay, the incomplete per-directory pass is skipped, the process still exits 0 — and a pre-flight check at the top of `_run_pipeline` makes **every later run** (scheduled, `--once`, `--refresh`) return immediately with an ERROR line until an operator runs `asr-crawler --reset-errors`. `--dry-run` (NullState) never counts; `--cleanup` zeroes the counter with the rest of the index. `per_step` mode completes the in-flight batch before the brake takes hold on the next run.
